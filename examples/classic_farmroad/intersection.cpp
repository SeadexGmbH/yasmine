...
const sxe::milliseconds TIMER_HIGHWAY_MINIMUM_TIME_DURATION( 5 );
const sxe::milliseconds TIMER_PHASE_1_DURATION( 1 );
const sxe::milliseconds TIMER_PHASE_2_DURATION( 1 );
const sxe::milliseconds TIMER_FARMROAD_MAXIMUM_TIME_DURATION( 3 );
...
intersection::~intersection() SX_NOEXCEPT
{
	// Nothing to do...
}
...
int intersection::fire_timed_event( const sxe::milliseconds _milliseconds, const sxy::event_sptr _event )
{
...
{
	SX_LOG( hermes::log_level::LL_INFO, "Highway closed." );
}


void intersection::switching_to_farmroad_phase_1()
{
	SX_LOG( hermes::log_level::LL_INFO, "Switching highway to farmroad. Phase 1." );
...
{
	SX_LOG( hermes::log_level::LL_INFO, "Switching highway to farmroad. Phase 2." );
...
{
	SX_LOG( hermes::log_level::LL_INFO, "Farmroad closed." );
}


void intersection::minimum_time_not_elapsed() const
{
	SX_LOG( hermes::log_level::LL_INFO, "Minimum time not elapsed." );
}
...
{
	SX_LOG( hermes::log_level::LL_INFO, "Minimum time not elapsed. Farmroad waiting." );
}


void intersection::switching_to_highway_phase_1()
{
	SX_LOG( hermes::log_level::LL_INFO, "Switching farmroad to highway. Phase 1." );
...
{
	SX_LOG( hermes::log_level::LL_INFO, "Switching farmroad to highway. Phase 2." );
...
{	
	if( _event.get_id() == detector_off_event::get_event_id() )
	{
		if( farmroad_maximum_time_event_handle_ > 0 )
		{
			...
			{
				SX_LOG( hermes::log_level::LL_INFO, "Event with handle % could not be canceled.", farmroad_maximum_time_event_handle_ );
			}
...
void intersection::build_intersection_state_machine()
{
	...
	sxy::simple_state& state_switching_to_farmroad_phase_2 = switching_to_farmroad_region.add_simple_state(
		"switching to farmroad phase 2", SX_NULLPTR, 
