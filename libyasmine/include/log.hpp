...
//!\brief Macro that is used when Y_NO_LOGGING is defined and thus there should be no logging. Consumes all parameters
...
//!\brief Macro to create a hex log message.
//!\param _level The log level of the message.
...
#define Y_LOG_HEX( _level, _heading, _data, ... ) \
	...
	{ \
		...
		{ \
			...
			l_log_manager.log( _level, sxy::log_manager::get_instance().get_timestamp(), __FILE__, __LINE__, l_message, __VA_ARGS__ ); \
		} \
	} \
	while( false )


//!\brief Macro to create a log message and wait for the message to be written by the logger.
//!\param _level The log level of the message.
//!\param ... Parameters that are inserted into the message.
//!\sa yprintf
#define Y_LOG_AND_WAIT( _level, ... ) \
	do \
	{ \
		sxy::log_manager_template<sxy::std_timestamp_policy>& l_log_manager = sxy::log_manager::get_instance(); \
		if( _level <= l_log_manager.get_log_level() ) \
		{ \
			l_log_manager.log_and_wait( _level, sxy::log_manager::get_instance().get_timestamp(), __FILE__, __LINE__, __VA_ARGS__ ); \
		} \
	} \
	while( false )
...
#define Y_LOG_AND_WAIT sxy::log_helper( __FILE__, __LINE__).log_and_wait
