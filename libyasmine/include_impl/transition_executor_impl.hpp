...
private:
	void get_all_parent_states_to_enter( raw_states_by_nesting_level& _states_to_enter, choice& _choice, 
		const vertex& source_vertex ) const;
	void fill_vector_of_states_to_exit( region* _LCA_region, raw_states_by_nesting_level_ascending& _states_to_exit, 
		const transition_kind _compound_transition_kind );
};
