...
{	
	...
	{
		p_defects.push_back( std::make_unique< t_state_machine_defect>( *this, "Choice '%' has no incoming transitions!", get_name() ) );
		l_check_ok = false;
	}
	Y_LOG( t_log_level::LL_SPAM, "Choice '%' have % incoming transition(s).", get_name(), get_incoming_transitions().size() );

	Y_LOG( t_log_level::LL_SPAM, "Checking if choice '%' have outgoing transition(s).", get_name() );
	if( get_outgoing_transitions().empty() )
	{
		p_defects.push_back( std::make_unique< t_state_machine_defect>( *this, "Choice '%' has no outgoing transitions!", get_name() ) );
		l_check_ok = false;
	}
	else
	{
		Y_LOG( t_log_level::LL_SPAM, "Choice '%' have % outgoing transition(s).", get_name(), get_outgoing_transitions().size() );
		Y_LOG( t_log_level::LL_SPAM, "Checking if choice '%' have more then one outgoing transition with no guard.", get_name() );
		...
		{
			p_defects.push_back( std::make_unique< t_state_machine_defect>( *this, "Choice '%' has more then one outgoing transition with no guard! It has % transitions with no guard.", get_name(), l_number_of_transitions_with_no_guard ) );
			l_check_ok = false;
		}

		Y_LOG( t_log_level::LL_SPAM, "Choice '%' have % outgoing transition(s) with no guard.", get_name(), l_number_of_transitions_with_no_guard );
	}
...
void
t_choice::accept_vertex_visitor
(
...
t_choice::accept_pseudostate_visitor
(
