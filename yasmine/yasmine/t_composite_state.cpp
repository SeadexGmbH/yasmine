...
t_composite_state::t_composite_state
(
	const std::string& p_name,
	i_behavior_uptr p_entry_action,
	i_behavior_uptr p_exit_action,
...
t_composite_state::add_region
(
	i_region_uptr p_region
)
{
...
t_composite_state::get_deep_history
(
) const
{
...
i_deep_history&
// cppcheck-suppress unusedFunction
t_composite_state::add_deep_history
(
	i_deep_history_uptr p_deep_history
)
{
	if(m_deep_history)
	{
...
t_composite_state::add_deep_history
(
	const std::string& p_deep_history_name
)
{
	if( m_deep_history )
	{
...
i_shallow_history*
t_composite_state::get_shallow_history
(
) const
{
...
i_shallow_history&
// cppcheck-suppress unusedFunction
t_composite_state::add_shallow_history
(
	i_shallow_history_uptr p_shallow_history
)
{
...
t_composite_state::add_shallow_history
(
	const std::string& p_shallow_history_name
)
{
	if( m_shallow_history )
	{
...
const t_raw_const_entry_points
t_composite_state::get_entry_points
(
)	const
{
...
i_entry_point&
// cppcheck-suppress unusedFunction
t_composite_state::add_entry_point
(
	i_entry_point_uptr p_entry_point
)
{
...
t_composite_state::add_entry_point
(
	const std::string& p_entry_point_name
)
{
...
const t_raw_const_exit_points
t_composite_state::get_exit_points
(
)	const
{
...
i_exit_point&
// cppcheck-suppress unusedFunction
t_composite_state::add_exit_point
(
	i_exit_point_uptr p_exit_point
)
{
...
t_composite_state::add_exit_point
(
	const std::string& p_exit_point_name
)
{
...
i_vertex*
// cppcheck-suppress unusedFunction
t_composite_state::get_pseudostate
(
...
size_t
t_composite_state::get_region_index
(
	const i_region* const p_region
) const
{
	auto l_element = std::find_if( m_regions.begin(), m_regions.end(), [ p_region ] ( const i_region_uptr& p_vector_element_region )
		{
...
size_t
t_composite_state::get_parent_region_index
(
) const
{
	size_t l_index = 0;
...
) const
{
	...
	{
		p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Composite state '%' has no regions!", get_name() ) );
		l_check_ok = false;
	}
	...
	{
		p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Simple state '%' defer event that triggers a outgoing transition!", get_name() ) );
		l_check_ok = false;
	}
...
void
t_composite_state::accept_vertex_visitor
(
...
void
t_composite_state::accept_complex_state_visitor
(
...
void
// cppcheck-suppress unusedFunction
t_composite_state::accept_state_visitor
(
...
bool
t_composite_state::is_orthogonal
(
) const
{
...
bool
t_composite_state::check_if_regions_are_completed
(
) const
{
