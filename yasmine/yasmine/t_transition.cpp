...
(
	...
	i_constraint_uptr p_guard,
	i_behavior_uptr p_behavior
):	
	t_transition( t_event_ids	{		p_event_id	}, p_source, p_target, p_kind, p_name, std::move( p_guard ), std::move( p_behavior ) )
{
...
sxy::t_transition_kind
t_transition::get_kind
(
) const
{
	return( m_kind );
...
void
t_transition::on_transition_behavior
(
	const i_event& p_event
) const
{
	Y_LOG( sxy::t_log_level::LL_INFO, "Executing transition '%' from '%' to '%'.", get_name().c_str(), get_source().get_name(), get_target().get_name() );
	...
	Y_LOG( sxy::t_log_level::LL_INFO, "Executed transition '%' from '%' to '%'.", get_name().c_str(), get_source().get_name(), get_target().get_name() );
}


bool
t_transition::check_guard
(
	const i_event& p_event
) const
{
...
) const
{
	...
	if( ( ( **found_source_itr ).get_uri().to_string() == l_lca_of_source_target->get_uri().to_string() ) || ( ( **found_target_itr ).get_uri().to_string() == l_lca_of_source_target->get_uri().to_string() ) )
	{
	...
	if( ( **found_source_itr ).get_uri().to_string() != ( **found_target_itr ).get_uri().to_string() )
	{
		...
		{
			p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Transition '%' has source ('%') and target ('%') in different regions of the same composite state.", get_name(), l_source.get_name(), l_target.get_name() ) );
			l_check_ok = false;
		}
	}
	...
	if( sxy::t_transition_kind::INTERNAL == get_kind() )
	{
		...
		if( l_source_state )
		{
			...
			{
				p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Transition '%' has '%' kind, but it has different states as source and target! (source= '%', target = '%').", get_name(), to_string( get_kind() ), m_source.get_name(), m_target.get_name() ) );
				l_check_ok = false;
			}
		}
		else
		{
			p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Source '%' of transition '%' (kind: '%') is not a state!", get_source().get_name(), get_name(), to_string( get_kind() ) ) );
			l_check_ok = false;
		}
	}
	...
	if( dynamic_cast< const i_pseudostate* >( &get_target() ) )
	{
		...
		{
			p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Transition '%' enters pseudostate '%' but have '%' kind! It is not possible!", get_name(), get_target().get_name(), to_string( get_kind() ) ) );
			l_check_ok = false;
		}
	}
	...
	if( t_transition_kind::LOCAL == get_kind() )
	{
		...
		{
			p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Transition's '%' source '%' and target '%' are not in a ancestor relationship!", get_name(), get_source().get_name(), get_target().get_name() ) );
			l_check_ok = false;
		}
	}
...
bool
t_transition::check_if_source_and_target_are_in_ancestor_relationship
(
...
bool
t_transition::check_relationship
(
