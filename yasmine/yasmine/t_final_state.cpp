...
t_final_state::t_final_state( const std::string& p_name )
	: t_state( p_name )
{
	// Nothing to do...
}
...
t_final_state::~t_final_state() = default;


i_behavior* t_final_state::get_entry_behavior() const
{
	return( nullptr );
}


i_behavior* t_final_state::get_exit_behavior() const
{
	return( nullptr );
}


const t_regions& t_final_state::get_regions() const
{
...
t_regions& t_final_state::get_regions()
{
...
bool t_final_state::check( t_state_machine_defects& p_defects ) const
{
	...
	{
		p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Final state '%' have '%' region(s)!",
	...
	{
		p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Final state '%' has entry behavior!",
				get_name() ) );
		l_check_ok = false;
	}
	...
	{
		p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Final state '%' has exit behavior!",
				get_name() ) );
		l_check_ok = false;
	}
...
void t_final_state::accept_vertex_visitor( i_const_vertex_visitor& p_visitor ) const
{
	p_visitor.visit( *this );
}


void t_final_state::accept_vertex_visitor( i_vertex_visitor& p_visitor )
{
	p_visitor.visit( *this );
}


void t_final_state::accept_state_visitor( i_state_visitor& p_visitor ) const
{
	p_visitor.visit( *this );
}


bool t_final_state::is_event_deferred( const t_event_id& p_event_id ) const
{
