...
t_transition_finder::t_transition_finder() = default;


t_transition_finder::~t_transition_finder() = default;


void t_transition_finder::search_for_enabled_transitions_in_all_regions( const i_state& p_current_state, 
	const i_event& p_event,	t_compound_transitions& p_enabled_compound_transitions,	bool& p_event_is_deferred ) const
{
	search_for_transition( p_current_state, p_enabled_compound_transitions, p_event, p_event_is_deferred );
}


void t_transition_finder::search_for_enabled_completion_transitions_in_all_regions(	const i_state& p_current_state,
	t_compound_transitions& p_enabled_compound_transitions,	bool& p_event_is_deferred ) const
{
...
void t_transition_finder::search_initial_transitions(	const i_composite_state& p_state,	
	t_compound_transitions& p_compound_transitions )
{
	for( const auto & l_region : p_state.get_regions() )
	{
		...
		{
			Y_LOG( t_log_level::LL_TRACE, "Initial state '%' found in region '%'.",
...
void t_transition_finder::search_choice_transitions( const t_raw_const_choices& p_choices, 
	t_compound_transitions& p_compound_transitions,	const i_event& p_event )
{
	for( const auto l_choice : p_choices )
	{
		...
		Y_LOG( t_log_level::LL_SPAM, "Choice '%' has % outgoing transition(s).",
			l_choice->get_name(), l_choice->get_outgoing_transitions().size() );
		auto l_at_least_one_transition_is_enabled = false;

		for( auto & l_transition : l_choice->get_outgoing_transitions() )
		{
			Y_ASSERT( l_transition->can_accept_event(
					COMPLETION_EVENT ), "Transition leaving choice is not a completion transition!" );
			Y_LOG( t_log_level::LL_SPAM, "Checking outgoing transition '%' of choice '%' for guard.",
				l_transition->get_name(), l_choice->get_name() );
			const auto guard_evaluated_to_true = l_transition->check_guard( p_event );
			if( guard_evaluated_to_true )
			{
				Y_LOG( t_log_level::LL_SPAM, "Guard of outgoing transition '%' of choice '%' evaluates to true.",
					l_transition->get_name(), l_choice->get_name() );
			...
			{
				Y_LOG( t_log_level::LL_SPAM, "Guard of outgoing transition '%' of choice '%' evaluates to false.",
					l_transition->get_name(), l_choice->get_name() );
			}
		}
...
i_transition* t_transition_finder::search_completion_transition( const i_state& p_state )
{
...
bool t_transition_finder::search_for_transition( const i_state& p_current_state, 
	t_compound_transitions& p_enabled_compound_transitions,	const i_event& p_event,	bool& p_event_is_deferred ) const
{
	...
	for( const auto & region : regions )
	{
		...
		if( active_state )
		{
			Y_LOG( t_log_level::LL_TRACE, "Found active state '%' in region '%'.", active_state->get_name(),
				region->get_name() );
			auto l_found =
				search_for_transition( *active_state, p_enabled_compound_transitions, p_event, p_event_is_deferred );
			if( l_found )
			{
	...
	if( !found )
	{
		...
		{
			Y_LOG( t_log_level::LL_TRACE, "Search transition with event id % in state '%'.",
				p_event.get_id(), p_current_state.get_name() );
			transition = p_current_state.search_transition( p_event );
		}

		if( transition )
		{
			Y_LOG( t_log_level::LL_TRACE, "Transition '%' found in the current state '%'.",
				transition->get_name(), p_current_state.get_name() );
