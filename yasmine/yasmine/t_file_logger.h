...
// location of the log files can be customized.
...
public:
	...
	t_file_logger( const size_t p_max_file_size_in_bytes,	const std::string& p_log_files_directory,	
	...
	virtual ~t_file_logger() = default;
	t_file_logger( const t_file_logger& ) = delete;
	t_file_logger& operator=( const t_file_logger& ) = delete;
...
private:
	std::string get_log_file_name_with_index( const unsigned p_index ) const;
	std::string get_working_log_file_name() const;
	void log_into_file( const std::string& p_message ) const;
	void rotate_if_necessary() const;
	void rename_files() const;
	void rename_file_if_exists(	const std::string& p_old_name, const std::string& p_new_name ) const;
	void delete_file( const std::string& p_file_name ) const;
