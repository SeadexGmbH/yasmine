...
namespace sxy
{
t_transition_controller::t_transition_controller() = default;
t_transition_controller::~t_transition_controller() = default;
bool t_transition_controller::process_event( const i_event& p_event, const i_composite_state& p_main_composite_state,
	i_event_processing_callback* const p_event_processing_callback,	bool& p_event_is_deferred )
{
	...
	transition_finder.search_for_enabled_transitions_in_all_regions( p_main_composite_state, p_event,
		enabled_compound_transitions, p_event_is_deferred );
	if( enabled_compound_transitions.empty() )
	{
		Y_LOG( sxy::t_log_level::LL_TRACE, "There are no compound transition to execute for the event '%'.",
			p_event.get_id() );
	}
	else
	{
		Y_LOG( t_log_level::LL_TRACE, "Found % enabled transition(s) for event '%'.",
			enabled_compound_transitions.size(), p_event.get_id() );
		Y_LOG( t_log_level::LL_SPAM,  "Start executing transition(s) for event '%'.", p_event.get_id() );
		l_reached_terminate_pseudostate = execute_transitions( p_main_composite_state, enabled_compound_transitions,
			p_event_processing_callback, p_event );
...
bool t_transition_controller::start_state_machine( const i_composite_state& p_main_composite_state,
	i_event_processing_callback* const p_event_processing_callback )
{
	...
	{
		Y_LOG( t_log_level::LL_TRACE, "Found % compound transition(s) after searching for initial transitions.",
			enabled_compound_transitions.size() );
		t_event event( COMPLETION_EVENT );
		Y_LOG( t_log_level::LL_TRACE, "Executing transitions." );
		if( execute_transitions( p_main_composite_state, enabled_compound_transitions, p_event_processing_callback,
					event ) )
		{
...
bool t_transition_controller::execute_transitions( const i_composite_state& p_main_composite_state,
	...
	const i_event& p_event )
{
	...
	while( !compound_transitions.empty() )
	{
		...
		l_terminate_pseudostate_has_been_reached = transition_executor.check_sort_and_execute_transitions(
			compound_transitions, l_choices, p_event_processing_callback, p_event );
		...
		Y_LOG( t_log_level::LL_TRACE, "Found % transition(s) after searched for choice transitions.",
...
void t_transition_controller::search_choice_transitions( const t_raw_const_choices& p_choices, 
...
void t_transition_controller::search_completion_event_transitions( const i_composite_state& p_main_composite_state,
	t_compound_transitions& compound_transitions )
{
	auto l_event_is_deferred = false;
	t_transition_finder transition_finder;
	transition_finder.search_for_enabled_completion_transitions_in_all_regions( p_main_composite_state,	
		compound_transitions,	l_event_is_deferred );
