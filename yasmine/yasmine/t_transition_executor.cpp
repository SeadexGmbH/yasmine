...
bool
t_transition_executor::check_sort_and_execute_transitions
(
	const t_compound_transitions& p_compound_transitions,	
	t_raw_const_choices& p_vertices,
...
)
{
	...
	const auto& sorted_compound_transitions = m_transition_executor_impl->sort_compound_transitions( p_compound_transitions );
	...
	for( auto& compound_transition : sorted_compound_transitions )
	{
		...
		m_transition_executor_impl->find_states_to_enter_and_to_exit_and_calculate_execution_steps( *compound_transition, l_execution_steps, l_entered_regions, p_event );
		...
		l_terminate_pseudostate_has_been_reached = m_transition_executor_impl->run_execution_steps( l_execution_steps, p_event_processing_callback, p_event );
