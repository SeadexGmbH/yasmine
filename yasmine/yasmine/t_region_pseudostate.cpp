...
t_region_pseudostate::t_region_pseudostate( const std::string& p_name )
...
t_region_pseudostate::~t_region_pseudostate() = default;


const i_state_machine_element* t_region_pseudostate::get_parent() const
{
	return( get_parent_region() );
}


i_region* t_region_pseudostate::get_parent_region() const
{
	return( m_parent );
}


void t_region_pseudostate::set_parent_region( i_region* const p_parent_region )
{
...
t_raw_composite_states t_region_pseudostate::get_ancestors( i_composite_state* const p_final_ancestor ) const
{
	...
	if( parent_region )
	{
		...
		Y_LOG( t_log_level::LL_SPAM, "Found parent state '%' for region '%'.",
		...
		Y_LOG( t_log_level::LL_SPAM, "Found % ancestor(s) of '%'.", ancestors_of_parent_state.size(),
...
t_raw_regions t_region_pseudostate::get_ancestors_as_regions() const
{
