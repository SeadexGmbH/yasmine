...
void
t_try_to_build_compound_transition_visitor::visit
(
...
void
t_try_to_build_compound_transition_visitor::build_compound_transition_and_insert_in_container
(
)
{
...
void
t_try_to_build_compound_transition_visitor::check_if_join_is_active_and_was_not_processed_yet
(
	const i_join& p_join
)
{
	const auto& transition_was_already_used = check_if_transition_was_already_used( m_enabled_transition, m_enabled_compound_transitions );

	if( !transition_was_already_used )
	{
		const auto& all_incoming_transitions_enabled = p_join.check_if_all_source_states_of_incoming_transitions_are_active();
	...
	{
		// The compound transition for this join was already built somewhere else. So the transition search has to end in this branch.
		m_is_built = true;
	}
}
...
bool
t_try_to_build_compound_transition_visitor::check_if_transition_was_already_used
(
