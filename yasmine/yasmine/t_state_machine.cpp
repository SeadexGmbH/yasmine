...
t_state_machine::t_state_machine(	const std::string& p_name, 
...
i_composite_state& t_state_machine::get_root_state() const
{
...
i_transition& t_state_machine::add_transition( i_transition_uptr p_transition )
{
...
i_transition& t_state_machine::add_transition( const std::string& p_transition_name, const t_event_id p_event_id,	
	i_vertex& p_source, i_vertex& p_target, const sxy::t_transition_kind p_kind, const t_constraint_function& p_guard,
	const t_behavior_function& p_behavior )
{
	auto l_transition =
		std::make_unique< sxy::t_transition >( p_event_id, p_source, p_target, p_kind, p_transition_name,
			( p_guard ? ( t_constraint::create_constraint( p_guard ) ) : nullptr ),
			( p_behavior ? ( t_behavior::create_behavior( p_behavior ) ) : nullptr ) );
...
i_transition& t_state_machine::add_transition( const std::string& p_transition_name, const t_event_ids p_event_ids,
	i_vertex& p_source,	i_vertex& p_target,	const sxy::t_transition_kind p_kind, const t_constraint_function& p_guard,
	const t_behavior_function& p_behavior )
{
	auto l_transition =
		std::make_unique< sxy::t_transition >( p_event_ids, p_source, p_target, p_kind, p_transition_name,
			( p_guard ? ( t_constraint::create_constraint( p_guard ) ) : nullptr ),
			( p_behavior ? ( t_behavior::create_behavior( p_behavior ) ) : nullptr ) );
	auto& transition = *l_transition;
	m_transitions.push_back( std::move( l_transition ) );
...
bool t_state_machine::fire_event( const t_event_sptr& p_event )
{
...
bool t_state_machine::check( t_state_machine_defects& p_defects ) const
{
...
bool t_state_machine::start_state_machine()
{
	Y_ASSERT( m_root_state, "No root state!" );
	...
	try
	{
		...
		l_terminate_pseudostate_has_been_reached = transition_controller.start_state_machine( *m_root_state,
			m_event_processing_callback );
		if( m_event_processing_callback )
		{
...
bool t_state_machine::process_event( const t_event_sptr& p_event )
{
	...
	try
	{
		...
		l_terminate_pseudostate_has_been_reached = transition_controller.process_event( *p_event, *m_root_state,
			m_event_processing_callback, l_event_is_deferred );
...
// cppcheck-suppress unusedFunction
const t_events& t_state_machine::get_deferred_events() const
{
...
// cppcheck-suppress unusedFunction
t_raw_const_states t_state_machine::get_active_state_configuration() const
{
...
void t_state_machine::get_active_states_from_region( t_raw_const_states& p_active_state_configuration,
	const i_region& p_region ) const
{
...
void t_state_machine::check_regions_for_active_states( t_raw_const_states& p_active_state_configuration,
	const i_state& p_state ) const
{
...
void t_state_machine::add_deferred_event( const t_event_sptr& p_event_id )
{
	m_deferred_events.push_back( p_event_id );
}


bool t_state_machine::process_deferred_events()
{
	...
	for( const auto l_deferred_event : l_deferred_events )
	{
		...
		auto l_event_is_deferred = false;
		l_terminate_pseudostate_has_been_reached = transition_controller.process_event( *l_deferred_event, *m_root_state,
			m_event_processing_callback, l_event_is_deferred );
		if( m_event_processing_callback )
		{
