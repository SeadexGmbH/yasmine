...
t_vertex::t_vertex( const std::string& p_name )
...
t_vertex::~t_vertex() = default;


void t_vertex::add_outgoing_transition( i_transition& p_outgoing_transition )
{
	m_outgoing_transitions.push_back( &p_outgoing_transition );
}


void t_vertex::add_incoming_transition( i_transition& p_incoming_transition )
{
	m_incoming_transitions.push_back( &p_incoming_transition );
}


void t_vertex::remove_outgoing_transition( const i_transition& p_outgoing_transition )
{
	auto outgoing_transition = std::find( m_outgoing_transitions.begin(),
		m_outgoing_transitions.end(), &p_outgoing_transition );
	m_outgoing_transitions.erase( outgoing_transition );
}


void t_vertex::remove_incoming_transition( const i_transition& p_incoming_transition )
{
	auto incoming_transition = std::find( m_incoming_transitions.begin(),
		m_incoming_transitions.end(), &p_incoming_transition );
	m_incoming_transitions.erase( incoming_transition );
}


const t_raw_transitions& t_vertex::get_outgoing_transitions() const
{
	return( m_outgoing_transitions );
}


const t_raw_transitions& t_vertex::get_incoming_transitions() const
{
	return( m_incoming_transitions );
}


t_uri t_vertex::get_uri() const
{
...
i_transition* t_vertex::search_transition( const i_event& p_event ) const
{
...
i_region* t_vertex::LCA_region( const i_vertex& p_rhs ) const
{
...
i_composite_state* t_vertex::LCA_composite_state( const i_vertex& p_rhs ) const
{
...
void t_vertex::add_ancestor_uri( t_uri& p_uri ) const
{
