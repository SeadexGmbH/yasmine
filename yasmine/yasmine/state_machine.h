...
#include "state_machine_introspection.h"
...
public:
	...
	virtual ~state_machine() override;
	...
	// !\param _transition_name Name of the transition.
	...
	virtual transition& add_transition(	const std::string& _transition_name,	const event_id _event_id, 
		vertex& _source,	vertex& _target,	const sxy::transition_kind _kind = transition_kind::EXTERNAL,
	...
	// !\param _behavior The behavior of the transition. Default is an empty function.
	// !\return a reference to the new created transition.
	virtual transition& add_transition(	const std::string& _transition_name,	const event_ids _event_ids,
		vertex& _source,	vertex& _target,	const sxy::transition_kind _kind = sxy::transition_kind::EXTERNAL,
		const constraint_function& _guard = constraint_function(),
		const behavior_function& _behavior = behavior_function() );
...
private:
	...
	void get_active_states_from_region(	raw_const_states& _active_state_configuration, 
		const region& _region ) const;
	void check_regions_for_active_states(	raw_const_states& _active_state_configuration, 
	...
	raw_const_transitions outgoing_transitions_;
	raw_const_transitions incoming_transitions_;
