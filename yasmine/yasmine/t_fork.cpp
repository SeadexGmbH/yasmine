...
t_fork::t_fork( const std::string& p_name )
...
t_fork::~t_fork() = default;


bool t_fork::check( t_state_machine_defects& p_defects ) const
{
	...
	for( const auto & l_transition : get_outgoing_transitions() )
	{
		...
		{
			p_defects.push_back( std::make_unique< t_state_machine_defect >( *this,
					"Outgoing transition '%' of fork '%' has guard!", l_transition->get_name(), get_name() ) );
			l_check_ok = false;
		}
	}
	...
	for( const auto & l_transition : get_outgoing_transitions() )
	{
		...
		if( !target_vertex )
		{
			p_defects.push_back( std::make_unique< t_state_machine_defect >( *this,
					...
					l_transition->get_name(), target_vertex->get_name() ) );
			l_check_ok = false;
		}
		else
		{
...
void t_fork::accept_vertex_visitor( i_const_vertex_visitor& p_visitor ) const
{
	p_visitor.visit( *this );
}


void t_fork::accept_vertex_visitor( i_vertex_visitor& p_visitor )
{
	p_visitor.visit( *this );
}


void t_fork::accept_pseudostate_visitor( i_pseudostate_visitor& p_visitor ) const
{
	p_visitor.visit( *this );
}


void t_fork::add_incoming_transition( i_transition& p_incoming_transition )
{
