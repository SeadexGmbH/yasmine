...
public:
	...
	explicit t_timed_event_creator( t_state_machine& p_state_machine );
	~t_timed_event_creator();
	t_timed_event_creator( const t_timed_event_creator& ) = delete;
	t_timed_event_creator& operator=( const t_timed_event_creator& ) = delete;
	...
	// !\return Handle of the new event creation request. The handle can be used for canceling the request.
	// !\sa cancel()
	int create_event_creation_request( const std::chrono::milliseconds& p_time,	const t_event_sptr& p_event );
	...
	// !\sa create_event()
	bool cancel( const int p_handle );
...
private:
	...
	bool check_if_handle_exists( const int p_handle ) const;
	void generate_event();
	t_event_queue::const_iterator find_element_by_handle( const int p_handle ) const;
