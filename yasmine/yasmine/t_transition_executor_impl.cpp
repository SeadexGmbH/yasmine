...
t_transition_executor_impl::t_transition_executor_impl() = default;


t_transition_executor_impl::~t_transition_executor_impl() = default;


void t_transition_executor_impl::get_active_states_from_region( i_region& p_region,	
	t_raw_states_by_nesting_level_ascending& p_states )
{
	Y_LOG( t_log_level::LL_SPAM, "Checking for active state in region '%' of state '%'.",
		p_region.get_name(), p_region.get_parent_state().get_name() );
	auto l_active_state = p_region.get_active_state();
	if( l_active_state )
	{
		Y_LOG( t_log_level::LL_SPAM, "Active state '%' found in region '%'.", l_active_state->get_name(),
			p_region.get_name() );
...
void t_transition_executor_impl::get_active_states_from_regions( const i_state* const p_state,
	t_raw_states_by_nesting_level_ascending& p_states )
{
	for( const auto & l_region : p_state->get_regions() )
	{
		Y_LOG( t_log_level::LL_SPAM, "Checking for active state in region '%' of state '%'.",
			l_region->get_name(), p_state->get_name() );
		auto l_active_state = l_region->get_active_state();
		if( l_active_state )
		{
			Y_LOG( t_log_level::LL_SPAM, "Active state '%' found in region '%'.",
				l_active_state->get_name(), l_region->get_name() );
			get_active_states_from_regions( l_active_state, p_states );
			p_states.insert( l_active_state );
		}
		else
...
void t_transition_executor_impl::get_all_states_to_enter_from_regions_that_are_not_explicitly_entered(
	i_compound_transition_consumer& p_compound_transition, t_raw_const_region_set& p_entered_regions,
	t_raw_states_by_nesting_level& p_states_to_enter,	const i_event& p_event )
{
	for( const auto l_state : p_states_to_enter )
	{
		for( const auto & l_region : l_state->get_regions() )
		{
			...
			else
			{
				...
				{
					Y_LOG( t_log_level::LL_DEBUG, "Found initial pseudostate '%' in the region '%'.",
						l_initial_pseudostate->get_name(), l_region->get_name() );
					...
					const auto built_compound_transition = l_new_compound_transition->create_and_check_transition_path(
						*l_initial_pseudostate->get_transition(), p_event );
					Y_ASSERT( built_compound_transition,
						"Transition of initial pseudostate could not be built in composite state!" );
					if( built_compound_transition )
					{
						Y_LOG( t_log_level::LL_DEBUG, "Build subcompound transition for '%' transition.",
							l_initial_pseudostate->get_name() );
...
void t_transition_executor_impl::merge_transitions_steps_with_exit_state_steps(	t_execution_steps& p_execution_steps,	
	i_compound_transition_consumer& p_compound_transition, 
	const t_raw_states_by_nesting_level_ascending& p_states_to_exit, 
	t_transition_steps::const_iterator& p_transition_start,	t_transition_steps::const_iterator& p_transition_end )
{
	...
	if( !p_states_to_exit.empty() )
	{
		...
		while( p_transition_end != p_compound_transition.get_transition_steps().end() &&
					 l_state_to_exit_end != p_states_to_exit.end() )
		{
			...
			if( l_exit_point )
			{
				...
				Y_LOG( t_log_level::LL_SPAM, "Found exit point '%' with parent state '%'.",
					l_exit_point->get_name(), l_exit_point_parent_state.get_name() );
				auto l_state_from_list = *( l_state_to_exit_end );
				Y_LOG( t_log_level::LL_SPAM, "The state from the list of 'states to exit' is '%'",
					l_state_from_list->get_name() );
				while( l_state_from_list != &l_exit_point_parent_state )
				{
					Y_LOG( t_log_level::LL_SPAM, "Add state '%' to execution steps as 'state to exit'.",
						l_state_from_list->get_name() );
...
void t_transition_executor_impl::merge_transitions_steps_with_enter_states_steps( t_execution_steps& p_execution_steps,
	i_compound_transition_consumer& p_compound_transition, const t_raw_states_by_nesting_level& p_states_to_enter,
	t_transition_steps::const_iterator& p_transition_start,	t_transition_steps::const_iterator& p_transition_end )
{
	if( !p_states_to_enter.empty() )
	{
		...
		while( p_transition_end != p_compound_transition.get_transition_steps().end() &&
					 l_state_to_enter_end != p_states_to_enter.end() )
		{
...
void t_transition_executor_impl::calculate_execution_steps(	i_compound_transition_consumer& p_compound_transition,
	const t_raw_states_by_nesting_level_ascending& p_states_to_exit, 
	const t_raw_states_by_nesting_level& p_states_to_enter,	t_execution_steps& p_execution_steps,
	t_raw_const_region_set& p_entered_regions, const i_event& p_event )
{
	Y_LOG( t_log_level::LL_SPAM, "Compound transition has % step(s).", 
		p_compound_transition.get_transition_steps().size() );
	...
	merge_transitions_steps_with_exit_state_steps( p_execution_steps, p_compound_transition, p_states_to_exit,
		l_transition_start, l_transition_end );
	if( l_transition_end == p_compound_transition.get_transition_steps().end() )
	{
		l_transition_end = l_transition_start;
	}

	merge_transitions_steps_with_enter_states_steps( p_execution_steps, p_compound_transition, p_states_to_enter,
		l_transition_start, l_transition_end );

	for( const auto & l_sub_compound_transition : p_compound_transition.get_sub_compound_transitions() )
	{
		Y_LOG( t_log_level::LL_SPAM, "Compound transition has % subcompound transition(s).",
			p_compound_transition.get_sub_compound_transitions().size() );
		find_states_to_enter_and_to_exit_and_calculate_execution_steps( *l_sub_compound_transition, p_execution_steps,
			p_entered_regions, p_event );
	}
}


bool t_transition_executor_impl::run_execution_steps(	const t_execution_steps& p_execution_steps,
	i_event_processing_callback* const p_event_processing_callback,	const i_event& p_event )
{
...
void t_transition_executor_impl::conflict_check( const t_compound_transitions& p_compound_transitions ) const
{
	...
	for( const auto & l_compound_transition : p_compound_transitions )
	{
		...
		if( l_state )
		{
			...
			check_conflicts_from_source_state_up_to_LCA( *l_state, unique_exit_states,
				&l_compound_transition->get_LCA_region()->get_parent_state() );
		}
	}
}


t_raw_compound_transitions t_transition_executor_impl::sort_compound_transitions(
...
void t_transition_executor_impl::find_all_states_to_exit(	i_compound_transition_consumer& p_compound_transition,
	t_raw_states_by_nesting_level_ascending& p_states_to_exit )
{
	...
	if( t_transition_kind::INTERNAL != l_compound_transition_kind )
	{
		...
		if( t_transition_kind::LOCAL == l_compound_transition_kind )
		{
			...
			{
				Y_LOG( t_log_level::LL_TRACE, "Least common ancestor '%' of compound transition, has no active state. .",
					l_least_common_ancestor->get_name() );
			}
		}
	}
	else
...
void t_transition_executor_impl::find_all_states_to_enter( i_compound_transition_consumer& p_compound_transition,
	t_raw_states_by_nesting_level& p_states_to_enter,	t_raw_const_region_set& p_regions_to_enter,	
	const i_event& p_event )
{
	...
	if( t_transition_kind::INTERNAL != l_compound_transition_kind )
	{
		...
		for( auto & l_transition : l_last_transition_step->get_transitions() )
		{
			...
			if( t_transition_kind::LOCAL == l_compound_transition_kind )
			{
				auto l_active_state_as_composite_state =
					dynamic_cast< i_composite_state* >( p_compound_transition.get_LCA_region()->get_active_state() );
				if( l_active_state_as_composite_state )
				{
					t_states_to_enter_visitor l_visitor( p_states_to_enter, p_regions_to_enter,
																							 *l_active_state_as_composite_state );
			...
			{
				t_states_to_enter_visitor l_visitor( p_states_to_enter, p_regions_to_enter,
																						 p_compound_transition.get_LCA_region()->get_parent_state() );
				l_vertex.accept_vertex_visitor( l_visitor );
			}
		}

		get_all_states_to_enter_from_regions_that_are_not_explicitly_entered( p_compound_transition, p_regions_to_enter,
			p_states_to_enter, p_event );
	}
	else
	{
...
void t_transition_executor_impl::add_remaining_states_to_enter(	
	...
	const t_raw_states_by_nesting_level& p_states_to_enter,	t_execution_steps& p_execution_steps )
{
	...
	while( l_state_start != p_states_to_enter.end() )
	{
		...
		Y_LOG( t_log_level::LL_SPAM, "State '%' added to execution steps as 'state to enter'.",
			l_state_from_list->get_name() );
...
void t_transition_executor_impl::add_remaining_states_to_exit( 
	const t_raw_states_by_nesting_level_ascending::const_iterator p_state_start,
	const t_raw_states_by_nesting_level_ascending& p_states, t_execution_steps& p_execution_steps )
{
	t_raw_states_by_nesting_level_ascending::const_iterator l_state_start = p_state_start;
	while( l_state_start != p_states.end() )
	{
		auto l_state_from_list = *l_state_start;
		Y_LOG( t_log_level::LL_SPAM, "Add remaining state '%' to execution steps as 'state to exit'.",
			l_state_from_list->get_name() );
...
void t_transition_executor_impl::add_remained_transitions( t_transition_steps::const_iterator& p_transition_start,
	const t_transition_steps::const_iterator& p_transition_end,	t_execution_steps& p_execution_steps )
{
	while( p_transition_start != p_transition_end )
	{
		auto l_execution_transition_step = std::make_unique< t_execution_transition_step >( **p_transition_start );
...
void t_transition_executor_impl::fill_vector_of_choices( t_raw_const_choices& p_choices,
...
void t_transition_executor_impl::find_all_states_to_enter_and_exit( i_compound_transition& l_new_compound_transition,
	t_raw_const_region_set& p_entered_regions, const i_event& p_event )
{
...
void t_transition_executor_impl::find_states_to_enter_and_to_exit_and_calculate_execution_steps(
	i_compound_transition_consumer& p_compound_transition, t_execution_steps& p_execution_steps, 
	t_raw_const_region_set& p_entered_regions, const i_event& p_event )
{
	t_raw_states_by_nesting_level_ascending l_states_to_exit = {};
	...
	calculate_execution_steps( p_compound_transition, l_states_to_exit, l_states_to_enter, p_execution_steps,
		p_entered_regions, p_event );
	Y_LOG( t_log_level::LL_TRACE, "% execution step(s) calculated.", p_execution_steps.size() );
}


void t_transition_executor_impl::check_conflicts_from_source_state_up_to_LCA(	const i_state& p_state,	
	t_raw_const_state_set& p_unique_exit_states, const i_composite_state* p_LCA ) const
{
	Y_LOG( t_log_level::LL_TRACE, "Checking conflicts from source '%' up to lca '%'.",
		p_state.get_name(), p_LCA->get_name() );
	const auto insert_result = p_unique_exit_states.insert( &p_state );
	if( !insert_result.second )
	{
		LOG_AND_THROW( t_log_level::LL_FATAL, "There are conflicts. The intersection of states '%' and '%' is not empty.",
			p_state.get_name(), p_LCA->get_name() );
	}
	else
	{
		Y_LOG( t_log_level::LL_TRACE, "There are no conflicts. The intersection of states '%' and '%' is empty.",
			p_state.get_name(), p_LCA->get_name() );
	}
