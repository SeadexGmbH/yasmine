...
t_file_logger::t_file_logger( const size_t p_max_file_size_in_bytes, const std::string& p_log_files_directory, 
	const std::string& p_name_suffix, const std::string& p_name_extension, const unsigned p_max_file_number, 
...
void t_file_logger::log( const t_log_message& p_log_message )
{
	const auto l_log_level = log_level_to_string( p_log_message.m_log_level_message );
	std::stringstream log_message_stream;
	log_message_stream << std::setw( g_log_level_block_width ) << std::setfill( ' ' ) << std::left << l_log_level <<
		" " << p_log_message.m_log_time_stamp << " [" << p_log_message.m_log_file << "@" << p_log_message.m_log_line <<
		"]: " << p_log_message.m_log_message;
	log_into_file( log_message_stream.str() );
	rotate_if_necessary();
}


void t_file_logger::log_into_file( const std::string& p_message ) const
{
...
std::string t_file_logger::get_log_file_name_with_index( const unsigned p_index ) const
{
...
std::string t_file_logger::get_working_log_file_name() const
{
	return( m_log_files_directory + m_name_sufix + "." + m_name_extension );
}


void t_file_logger::rotate_if_necessary() const
{
...
void t_file_logger::rename_files() const
{
...
void t_file_logger::rename_file_if_exists( const std::string& p_old_name, const std::string& p_new_name ) const
{
...
void t_file_logger::delete_file( const std::string& p_file_name ) const
{
