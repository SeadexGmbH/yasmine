...
t_complex_state::t_complex_state
(		
...
i_behavior*
t_complex_state::get_entry_behavior
(
) const
{
	return( m_entry.get() );
...
i_behavior*
t_complex_state::get_exit_behavior
(
) const
{
	return( m_exit.get() );
...
bool
t_complex_state::is_event_deferred
(
	const t_event_id& p_event_id
) const
{
	const auto l_found = std::find( m_deferred_events.begin(), m_deferred_events.end(), p_event_id ) != m_deferred_events.end();
	return( l_found );
}


bool
t_complex_state::check_if_one_of_the_deferred_events_triggers_a_transition
(		
) const
{
