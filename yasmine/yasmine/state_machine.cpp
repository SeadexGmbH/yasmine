...
state_machine::state_machine( const std::string& _name,	event_processing_callback* const _event_processing_callback )
	: name_( _name ),
		...
		root_state_( Y_MAKE_UNIQUE< composite_state_impl >( _name ) ),
		transitions_(),
		deferred_events_(),
...
state_machine::~state_machine() Y_NOEXCEPT
{
#ifdef Y_PROFILER
...
sxy::uint32_t state_machine::get_number_of_processed_events() const
{
...
transition& state_machine::add_transition( transition_uptr _transition )
{
	...
	transitions_.push_back( sxy::move( _transition ) );
	return( transition );
}
...
{		
	Y_LOG( log_level::LL_INFO, "Firing & processing event '%' (%) with priority '%'.", _event->get_name(), 		
		_event->get_id(), static_cast<sxy::int8_t>( _event->get_priority() ));
	const bool terminate_pseudostate_has_been_reached = process_event( _event, Y_NULLPTR );
...
bool state_machine::check( state_machine_defects& _defects ) const
{
	...
	if( check_ok )
	{
		Y_FOR( const transition_uptr& transition, transitions_ )
		{
...
bool state_machine::start_state_machine()
{
	...
	const bool state_machine_started = start_state_machine( Y_NULLPTR );
...
bool state_machine::start_state_machine( async_event_handler* const _async_event_handler )
{
	...
	try
	{
		if( event_processing_callback_ )
		{
			event_processing_callback_->before_event( COMPLETION_EVENT_ID );
		}
		...
		if( event_processing_callback_ )
		{
			event_processing_callback_->after_event( COMPLETION_EVENT_ID );
		}
...
bool state_machine::process_event( const event_sptr& _event, async_event_handler* const _async_event_handler )
{
	...
	Y_LOG( log_level::LL_INFO, "'%' is processing event '%' (%) with priority '%'.", get_name(), _event->get_name(), 
		_event->get_id(), static_cast<sxy::int8_t>(_event->get_priority()) );
	...
	Y_LOG( log_level::LL_TRACE, "Starting processing of event '%' (%) with priority '%'.", _event->get_name(), _event->get_id(),
		static_cast<sxy::int8_t>(_event->get_priority()) );
	try
	{
		if( event_processing_callback_ )
		{
...
bool state_machine::process_deferred_events( async_event_handler* const _async_event_handler )
{
	...
	Y_FOR( const event_sptr deferred_event, deferred_events )
	{
		if( event_processing_callback_ )
		{
...
{		
	Y_FOR( region_uptr& region, _state.get_regions())
	{
