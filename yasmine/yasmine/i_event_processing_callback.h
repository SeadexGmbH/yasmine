...
{
public:
	i_event_processing_callback() = default;
	virtual ~i_event_processing_callback() = default;
	i_event_processing_callback( const i_event_processing_callback& ) = delete;
	...
	// !\return void
	virtual void add_state_machine_introspection( i_state_machine_introspection& p_state_machine ) = 0;

	// !\brief Callback before an event is processed.
	// !\param p_event_id ID of the event.
	// !\return void
	virtual void before_event( const t_event_id p_event_id ) = 0;

	// !\brief Callback after an event is processed.
	// !\param p_event_id ID of the event.
	// !\return void
	virtual void after_event( const t_event_id p_event_id ) = 0;
	...
	// !\return void
	virtual void after_event_processings_stage( const t_raw_const_choices& p_choices ) = 0;
