...
t_build_transition_steps_visitor::t_build_transition_steps_visitor( i_transition& p_current_transition, 
	t_transition_steps& p_transitions_steps, const i_event& p_event )
...
t_build_transition_steps_visitor::~t_build_transition_steps_visitor() = default;


void t_build_transition_steps_visitor::visit( const i_composite_state& p_composite_state )
{
...
void t_build_transition_steps_visitor::visit( const i_simple_state& p_simple_state )
{
...
void t_build_transition_steps_visitor::visit( const i_final_state& p_final_state )
{
...
void t_build_transition_steps_visitor::visit( const i_initial_pseudostate& p_initial_pseudostate )
{
...
void t_build_transition_steps_visitor::visit( const i_choice& p_choice )
{
...
void t_build_transition_steps_visitor::visit( const i_junction& p_junction )
{
	handle_as_junction( p_junction );
}


void t_build_transition_steps_visitor::visit( const i_join& p_join )
{
	handle_as_join( p_join );
}


void t_build_transition_steps_visitor::visit( const i_fork& p_fork )
{
	handle_as_fork( p_fork );
}


void t_build_transition_steps_visitor::visit( const i_entry_point& p_entry_point )
{
	handle_entry_point( p_entry_point );
}


void t_build_transition_steps_visitor::visit( const i_exit_point& p_exit_point )
{
	handle_exit_point( p_exit_point );
}


void t_build_transition_steps_visitor::visit( const i_deep_history& p_deep_history )
{
	handle_history_pseudostate( p_deep_history );
}


void t_build_transition_steps_visitor::visit( const i_shallow_history& p_shallow_history )
{
	handle_history_pseudostate( p_shallow_history );
}


void t_build_transition_steps_visitor::visit( const i_terminate_pseudostate& p_terminate_pseudostate )
{
...
i_transition* t_build_transition_steps_visitor::get_next_transition() const
{
	return( m_next_transition );
}


bool t_build_transition_steps_visitor::reached_end_of_transition() const
{
	return( m_reached_end_of_transition );
}


i_transition* t_build_transition_steps_visitor::find_next_transition( const i_pseudostate& p_target_pseudostate,
...
t_raw_transitions t_build_transition_steps_visitor::get_default_transition_if_state_was_not_active_before(
	const i_history& p_history )
{
	...
	if( !state_was_active_before )
	{
		...
		{
			LOG_AND_THROW( t_log_level::LL_FATAL, "History pseudostate '%' has no default transition!",
				p_history.get_name() );
		}
	}
...
void t_build_transition_steps_visitor::handle_history_pseudostate( const i_history& p_history )
{
	auto simple_step = std::make_unique< t_simple_transition_step >( m_current_transition );
	m_transition_steps.push_back( std::move( simple_step ) );
...
void t_build_transition_steps_visitor::handle_as_junction( const i_pseudostate& p_pseudostate )
{
	auto simple_step = std::make_unique< t_simple_transition_step >( m_current_transition );
	m_transition_steps.push_back( std::move( simple_step ) );
	...
	{
		LOG_AND_THROW( t_log_level::LL_FATAL, "There is no enabled transition for the junction '%'!",
			p_pseudostate.get_name() );
	}
}


void t_build_transition_steps_visitor::handle_as_fork( const i_pseudostate& p_pseudostate ) const
{
	auto simple_step = std::make_unique< t_simple_transition_step >( m_current_transition );
	m_transition_steps.push_back( std::move( simple_step ) );
...
void t_build_transition_steps_visitor::handle_as_join( const i_pseudostate& p_pseudostate )
{
...
void t_build_transition_steps_visitor::handle_entry_point( const i_entry_point& p_entry_point )
{
...
void t_build_transition_steps_visitor::handle_exit_point( const i_exit_point& p_exit_point )
{
...
void t_build_transition_steps_visitor::create_simple_transition_step()
{
	auto simple_step = std::make_unique< t_simple_transition_step >( m_current_transition );
	m_transition_steps.push_back( std::move( simple_step ) );
}
