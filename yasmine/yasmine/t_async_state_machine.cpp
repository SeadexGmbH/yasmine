...
{
	Y_ASSERT( ( t_status::NEW == m_status ) || ( t_status::STOPPED == m_status ), "Status is neither 'NEW' nor 'STOPPED' on destruction!" );
}


bool
t_async_state_machine::fire_event
(
...
void
t_async_state_machine::start
(
)
{
	Y_ASSERT( ( t_status::NEW == m_status ) || ( t_status::STOPPED == m_status ), "Status is neither 'NEW' nor 'STOPPED' on start!" );
...
void
t_async_state_machine::stop
(
)
{
	Y_LOG( t_log_level::LL_DEBUG, "Stopping state machine from run." );

	Y_ASSERT( ( t_status::STARTED == m_status ) || ( t_status::STOP_REQUESTED == m_status ), "Status is not 'STARTED' or 'STOP_REQUESTED' on stop!" );
...
bool
t_async_state_machine::wait_predicate
(
) const
{
	...
	try
	{
		while( true )
		{
			t_event_sptr l_event;
			{
				...
				m_run_and_event_condition.wait
				( 
					lock, [ this ] ()
					{
