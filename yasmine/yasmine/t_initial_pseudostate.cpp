...
t_initial_pseudostate::t_initial_pseudostate
(
...
{	
	...
	{
		p_defects.push_back( std::make_unique< t_state_machine_defect>( *this, "Initial pseudostate '%' has more then 1 outgoing transition! It has '%' transitions.", get_name(), get_outgoing_transitions().size() ) );
		l_check_ok = false;
	}
	...
	if( get_outgoing_transitions().size() != 1 )
	{			
		p_defects.push_back( std::make_unique< t_state_machine_defect>( *this, "Initial pseudostate '%' doesn't have exactly 1 outgoing transition! It has % transition(s).", get_name(), get_outgoing_transitions().size() ) );
		l_check_ok = false;
	}

	// 15.3.8 Pseudostate -> Constraint [9]: Outgoing transition from an initial vertex may have a behavior, but not a trigger or guard.
	for( const auto& l_transition : get_outgoing_transitions() )
	{
		if( l_transition->get_guard() )
		{	
			p_defects.push_back( std::make_unique< t_state_machine_defect>( *this, "Outgoing transition '%' of initial pseudostate '%' has a guard!", l_transition->get_name(), get_name() ) );
			l_check_ok = false;
		}
	}
...
void
t_initial_pseudostate::accept_vertex_visitor
(
...
void
t_initial_pseudostate::accept_pseudostate_visitor
(
...
i_transition*
t_initial_pseudostate::get_transition
(
) const
{		
...
void
t_initial_pseudostate::add_incoming_transition
(
