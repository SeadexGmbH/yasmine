...
const i_composite_state&
t_region::get_parent_state
(
) const
{		
...
size_t
t_region::get_pseudostate_count
(
)	const
{
...
t_raw_const_pseudostates
t_region::get_pseudostates
(
) const
{
...
t_region::get_initial_pseudostate
(
)	const
{
...
{		
	...
	if( p_active_state )
	{
		Y_LOG( t_log_level::LL_SPAM, "Set state '%' as 'active state' in region '%'.", p_active_state->get_name(), get_name() );
	}
	else
	{
...
void
t_region::set_state_was_active
(
	i_state* const p_active_state
)
{
	m_last_active_state = p_active_state;
	Y_LOG( t_log_level::LL_SPAM, "Set state '%' as 'was active state' in region '%'.", p_active_state->get_name(), get_name() );
}


const i_state*
t_region::get_last_active_state
(
) const
{
...
i_initial_pseudostate&
// cppcheck-suppress unusedFunction
t_region::add_initial_pseudostate
(	
...
{		
	...
	auto l_simple_state = std::make_unique< sxy::t_simple_state >
		(
			p_name,
			std::move( l_behavior ),
			std::move( l_entry_behavior ),
...
)
{
	auto l_simple_state = std::make_unique< sxy::t_simple_state >
		(
			...
			( p_behavior == nullptr ? nullptr : ( t_behavior::create_behavior( p_exit_behavior ) ) ),
			p_deferred_events
			);
...
{
	auto l_composite_state = std::make_unique< sxy::t_composite_state >
		(
			p_name,
...
// cppcheck-suppress unusedFunction
t_region::add_terminate_pseudostate
(
	i_terminate_pseudostate_uptr p_terminate_pseudostate
)
{
...
bool
t_region::is_active_state_final
(
) const
{
...
) const
{
	...
	// 15.3.10 Region -> Constraint [2]: A region can have at most one deep history vertex. -> deep history belongs to composite state
	// 15.3.10 Region -> Constraint [3]: A region can have at most one shallow history vertex. -> shallow history belongs to composite state



	// 15.3.10 Region -> Constraint [4]: If a Region is owned by a StateMachine, then it cannot also be owned by a State and vice versa.
	// Enforced by design.
