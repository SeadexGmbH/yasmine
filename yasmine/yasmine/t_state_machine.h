...
class t_state_machine:
	private i_state_machine_introspection
{
public:
	...
	explicit t_state_machine(	const std::string& p_name, 
		i_event_processing_callback* const p_event_processing_callback = nullptr );
	virtual ~t_state_machine() override;
	t_state_machine( const t_state_machine& ) = delete;
	t_state_machine& operator=( const t_state_machine& ) = delete;
	...
	i_composite_state& get_root_state() const;
	...
	virtual i_transition& add_transition( i_transition_uptr p_transition );
	...
	virtual i_transition& add_transition(	const std::string& p_transition_name,	const t_event_id p_event_id, 
		i_vertex& p_source,	i_vertex& p_target,	const sxy::t_transition_kind p_kind = t_transition_kind::EXTERNAL,
	...
	// !\param p_behavior The behavior of the transition. Default is an empty function.
	// !\return a reference to the new created transition.
	virtual i_transition& add_transition(	const std::string& p_transition_name,	const t_event_ids p_event_ids,
		i_vertex& p_source,	i_vertex& p_target,	const sxy::t_transition_kind p_kind = sxy::t_transition_kind::EXTERNAL,
		const t_constraint_function& p_guard = t_constraint_function(),
		const t_behavior_function& p_behavior = t_behavior_function() );
	...
	// the state machine was stopped and no
	// further events can be fired.
	virtual bool fire_event( const t_event_sptr& p_event );
	...
	bool check( t_state_machine_defects& p_defects ) const;
...
protected:
	bool process_event( const t_event_sptr& p_event );


private:
	virtual const t_events& get_deferred_events() const override;
	virtual t_raw_const_states get_active_state_configuration() const override;
	void get_active_states_from_region(	t_raw_const_states& p_active_state_configuration, 
		const i_region& p_region ) const;
	void check_regions_for_active_states(	t_raw_const_states& p_active_state_configuration, 
		const i_state& p_state ) const;
	void add_deferred_event( const t_event_sptr& p_event_id );
	bool process_deferred_events();
