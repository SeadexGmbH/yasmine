...
int
t_timed_event_creator::create_event_creation_request
(
...
{	
...

	{
		...
		{
			LOG_AND_THROW( t_log_level::LL_FATAL, "Time based event creator is not running. Event creation requests cannot be created!" );			
		}		
	}
...
t_timed_event_creator::create_event_creation_request
(
	const std::chrono::milliseconds& p_time_till_event_is_fired,
	const t_event_sptr& p_event
)
{	
	const auto l_handle = create_event_creation_request( std::chrono::system_clock::now() + p_time_till_event_is_fired, p_event );
	return( l_handle );	
}
...
int
t_timed_event_creator::generate_handle
(
) const
{
	int l_handle = 0;
...
bool
t_timed_event_creator::check_if_handle_exists
(
	const int p_handle
) const
{
...
void
t_timed_event_creator::generate_event
(
)
{
	try
	{			
		...
		{		
			...
			{
				m_condition_variable.wait( l_lock, [ this ] () { return( !(m_run &&  m_event_creation_requests.empty()) ); } );
			}
			else
			{
				...
				if( m_run ) 
				{
					...
					while( l_event_iterator != m_event_creation_requests.end() )
					{
						...
						{								
							...
							Y_LOG( t_log_level::LL_DEBUG, "Try to fire event '%' with handle '%'.", l_event->get_id(), ( *l_event_iterator )->get_handle() );
							if( !m_state_machine.fire_event( l_event ) )
							{
								Y_LOG( t_log_level::LL_ERROR, "Event with id '%' and handle was not fired!", l_event->get_id(), ( *l_event_iterator )->get_handle() );

								m_run = false;
								break;
							}
							Y_LOG( t_log_level::LL_DEBUG, "Event '%' with handle '%' was fired.", l_event->get_id(), ( *l_event_iterator )->get_handle() );
...
t_timed_event_creator::find_element_by_handle
(
	const int p_handle
) const
{
	return( std::find_if( m_event_creation_requests.begin(), m_event_creation_requests.end(), [ &p_handle ] ( const std::unique_ptr<t_event_creation_request>& p_event_creation_request ) { return( p_event_creation_request->get_handle() == p_handle ); } ) );
}
