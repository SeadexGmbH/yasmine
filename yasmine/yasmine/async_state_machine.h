...
public:
	...
	virtual ~async_state_machine() override;
...
private:
	...
	bool enqueue( const event_sptr _event );
	bool enqueue_front( const event_sptr _event );
	bool enqueue_impl( const event_sptr _event, queue_inserter _queue_inserter );
	bool wait_predicate() const;
	void work();	
	
	// !\brief Sends a priority (internal) event and enqueues it in the front of the event queue, so it will be processed before other events.
	// !\return void
	...
	bool run_;
	std::deque< event_sptr > event_queue_;
};
