...
bool
t_join::check_if_all_source_states_of_incoming_transitions_are_active
(
) const
{
	...
	for( const auto& transition : incoming_transitions )
	{
		...
		all_incoming_transitions_srouces_are_active = check_if_all_incoming_transitions_sources_are_active_visitor.get_result();
...
void
t_join::accept_pseudostate_visitor
(
...
) const
{
	...
	// 15.3.8 Pseudostate -> Constraint [3]: A join vertex must have at least two incoming transitions and exactly one outgoing transition.
	if( get_incoming_transitions().size() < 2 )
	{
		p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Join has too few incoming transitions!" ) );
		l_check_ok = false;
	}

	if( get_outgoing_transitions().size() != 1 )
	{
		p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Join does not have exactly 1 outgoing transition!" ) );
		l_check_ok = false;
	}

	// 15.3.8 Pseudostate -> Constraint [4]: All transitions incoming a join vertex must originate in (math formula contains no "different") regions of an orthogonal state.
	...
	for( auto l_iterator_1 = transitions.begin(); l_iterator_1 < transitions.end(); ++l_iterator_1 )
	{
		for( auto l_iterator_2 = l_iterator_1 + 1; l_iterator_2 < transitions.end(); ++l_iterator_2 )
		{
			...
			if(!l_result.second)
			{
				p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Join '%' has incoming transitions that originate in same region of a composite state!", get_name() ) );
				l_check_ok = false;
				break;
			}
		}
	...
	for( const auto& l_transition : get_incoming_transitions() )
	{
		...
		if( !l_source_vertex )
		{
			p_defects.push_back( std::make_unique< t_state_machine_defect >( *this, "Join segment does not originate from a state!" ) );
			l_check_ok = false;
		}
		else
		{
...
void
t_join::add_outgoing_transition
(
