...
t_compound_transition::~t_compound_transition() = default;


t_transition_steps& t_compound_transition::get_transition_steps()
{
	return( m_transition_steps );
}


const i_vertex& t_compound_transition::get_last_target() const
{
...
const t_compound_transitions& t_compound_transition::get_sub_compound_transitions() const
{
	return( m_sub_compound_transitions );
}


void t_compound_transition::add_sub_compound_transition( i_compound_transition_uptr p_sub_compound_transition )
{
	m_sub_compound_transitions.push_back( std::move( p_sub_compound_transition ) );
}


i_region* t_compound_transition::get_LCA_region()
{
...
i_composite_state* t_compound_transition::get_LCA_composite_state()
{
	const auto& l_step1 = m_transition_steps.front().get();
	const auto& l_transition1 = l_step1->get_transitions().front();
	const auto& l_source = l_transition1->get_source();
...
t_transition_kind t_compound_transition::get_transition_kind()
{
...
bool t_compound_transition::check_if_starts_with( const i_transition& p_transition )
{
...
bool t_compound_transition::create_and_check_transition_path( i_transition& p_start_transition,	
