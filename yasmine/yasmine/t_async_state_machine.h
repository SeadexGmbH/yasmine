...
//!\brief Class for the "multi-threaded version" of the state machine. Provides methods to start and halt the state machine and to fire events.
class t_async_state_machine final: public t_state_machine
{
...
public:
	...
	explicit t_async_state_machine
	(
	...
	//!\return true if event was successfully enqueued for firing, else false. It returns false if the enqueue for firing of the event is not possible because 
	...
	//!\brief Starts the state machine.	When starting the state machine, the initial transitions are searched and executed. Without running the state machine,
	// it is not possible to fire events.
	//!\return If the state machine reaches a terminate pseudostate after starting (on calling run), returns true. Else if the state machine is started and running,
	// returns false.
	bool
	run
	(
	...
	//!\brief Stops the state machine. When stopping the state machine, all the events remaining in the queue of events will be fired and the event processing thread
